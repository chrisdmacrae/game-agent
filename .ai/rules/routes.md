...
## One MCP endpoint PER GAME, never a public/authenticated pair
Each game gets exactly ONE MCP endpoint (/mcp/poe2, /mcp/d4) — adding a game means adding one more `Mcp::web()` line, never a second authenticated URL beside an existing one. Mcp::oauthRoutes() serves OAuth discovery metadata for EVERY path (`/.well-known/oauth-protected-resource/{path}` with `.*`), so clients complete the OAuth login against a game's URL and then send their token to that same URL. Every game route must keep the AuthenticateIfBearerPresent:api middleware: anonymous requests stay read-only, a presented token authenticates for real and unlocks user-gated tools (save_build, via shouldRegister → Auth::check()), and an invalid token 401s (so clients refresh instead of silently degrading to read-only). A split public/authenticated endpoint pair caused exactly that silent degradation before.

Because the endpoint path is not derivable from the game slug (`diablo-4` lives at `/mcp/d4`), the frontend reads the per-game `mcpUrls` map shared by HandleInertiaRequests rather than rewriting the legacy single `mcpUrl` prop.
