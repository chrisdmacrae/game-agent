...
import type { GearViewItem } from '@/components/GearScreen.vue';
import PassiveTreeView from '@/components/PassiveTreeView.vue';
import SeoHead from '@/components/SeoHead.vue';
import { home } from '@/routes';
import { buildFile, ogImage } from '@/routes/builds';
...
const seoIdentity = [identity, def.level ? `level ${def.level}` : '']
    .filter(Boolean)
    .join(', ');
const seoDescription =
    props.build.summary ??
    (seoIdentity
        ? `${seoIdentity} build for Path of Exile 2.`
        : 'A build for Path of Exile 2.');
    ...
    >
        <div class="mx-auto max-w-3xl px-6 py-12">
            ...
            <header class="mt-6 mb-10">
                <div class="mb-3 flex flex-wrap items-center gap-2 text-sm">
                    <span
                        v-if="identity"
                        class="rounded-full bg-amber-500/10 px-3 py-1 font-medium text-amber-400"
                        >{{ identity }}</span
                    >
                    <span
                        v-if="def.level"
                        class="rounded-full bg-zinc-800 px-3 py-1 text-zinc-400"
                        >Level {{ def.level }}</span
                    >
                    <span
                        v-if="def.content_tier"
                        class="rounded-full bg-zinc-800 px-3 py-1 text-zinc-400 capitalize"
                        >{{ def.content_tier.replace('_', ' ') }}</span
                    >
                    <span
                        v-if="build.game_version"
                        class="rounded-full bg-zinc-800 px-3 py-1 text-zinc-400"
                        >Patch {{ build.game_version }}</span
                    >
                </div>
                <h1 class="text-3xl font-bold tracking-tight text-white">
                    {{ build.name }}
                </h1>
                <p v-if="build.summary" class="mt-2 text-lg text-zinc-400">
                    {{ build.summary }}
                </p>
                <div class="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    >
                        {{
                            pobCopied
                                ? 'Copied! Paste in PoB → Import from code'
            ...
            >
                <p class="mb-1 font-semibold">
                    This build has unresolved rule violations:
                </p>
            ...
            >
                ✓ Passes all game-rule checks (support limits, spirit budget,
                passive existence).
            </div>
            ...
            <section class="mb-10">
                ...
                >
                    <div
                        v-for="setup in def.skills"
                        :key="setup.gem"
                        class="flex flex-wrap items-center gap-x-3 gap-y-2 p-4"
                    >
                        <img
                        ...
                        <span
                            class="entity-ref font-semibold text-amber-400"
                            :data-entity="setup.gem"
                            >{{ setup.gem }}</span
                        >
                        <span
                            v-if="setup.supports?.length"
                            class="flex flex-wrap items-center gap-1.5 text-sm text-zinc-400"
                        >
                            ←
                            <span
            ...
            <section
                v-if="
                    Object.keys(gearView.slots).length || gearView.jewels.length
                "
                class="mb-10"
            >
            ...
            <section
                v-if="
                    def.passives?.keystones?.length ||
                    def.passives?.notables?.length ||
                    def.resistances
                "
            ...
            >
                ...
                >
                    ...
                    <ul class="space-y-2 text-sm">
                        <li
                            v-for="passive in [
                                ...(def.passives?.keystones ?? []),
                                ...(def.passives?.notables ?? []),
                            ]"
                            :key="passive"
                        ...
                        >
                            ...
                            <span
                                class="entity-ref"
                                :class="
                                    entityFor(passive)?.passive_kind ===
                                    'keystone'
                                        ? 'text-amber-400'
                                        : 'text-zinc-300'
                                "
                ...
                <div v-if="def.resistances">
                    ...
                    <ul class="space-y-1 text-sm text-zinc-300">
                        <li
                            v-for="(value, element) in def.resistances"
                            :key="element"
                            class="capitalize"
                        >
                            {{ element }}:
            ...
            >
                <h2 class="mb-3 text-lg font-semibold text-white">
                    ...
                    <span class="text-sm font-normal text-zinc-500">
                        {{
                            def.passives?.node_ids?.length
                                ? `— ${def.passives.node_ids.length} allocated nodes`
                ...
                <PassiveTreeView
                    ...
                    :granted-ids="
                        (def.passives?.granted_nodes ?? []).map(
                            (g) => g.node_id,
                        )
                    "
            ...
            <section v-if="build.guide_html" class="mb-10">
                ...
                <div
                    class="guide-content leading-relaxed text-zinc-300"
                    v-html="build.guide_html"
                />
            </section>

            <footer class="border-t border-zinc-800 pt-4 text-xs text-zinc-600">
                Saved {{ build.created_at }} · Build id {{ build.id }} ·
                Generated with an AI agent connected to the PoE2 Theorycrafter
                MCP server. Not affiliated with Grinding Gear Games.
            </footer>
        </div>
        ...
        <Transition name="fade">
            ...
            >
                <!-- Gem / support -->
                <template
                    v-if="hovered.kind === 'gem' || hovered.kind === 'support'"
                >
                    <div class="mb-1 flex items-center gap-2">
                        ...
                        <span
                            ...
                            :class="
                                gemColors[hovered.color ?? 'w'] ?? gemColors.w
                            "
                    ...
                    <p
                        v-if="hovered.description"
                        class="mb-2 text-sm text-zinc-400"
                    >
                        {{ hovered.description }}
                    </p>
                    <p
                        v-if="hovered.spirit_reservation"
                        class="mb-2 text-sm text-sky-300"
                    >
                        Reserves {{ hovered.spirit_reservation }} Spirit
                    </p>
                    <ul
                        v-if="hovered.stat_text?.length"
                        class="space-y-0.5 text-sm text-sky-200/80"
                    >
                        <li v-for="line in hovered.stat_text" :key="line">
                            {{ line }}
                        </li>
                    </ul>
                </template>
                    ...
                    <div class="mb-2 flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-zinc-500 capitalize">
                                {{ hovered.passive_kind }} passive
                            </p>
                        </div>
                    </div>
                    <ul class="space-y-0.5 text-sm text-sky-200/80">
                        <li
                            v-for="stat in hovered.stats"
                            :key="stat"
                ...
                <template v-else-if="hovered.kind === 'unique'">
                    ...
                    <ul class="space-y-0.5 text-sm text-sky-200/80">
                        <li v-for="mod in hovered.mods" :key="mod">
                            {{ mod }}
                        </li>
                    </ul>
                </template>
